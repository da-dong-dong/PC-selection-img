<template>
  <div class="all" @click="allFun($event)" v-on:keyup.delete="delFun">
    <div>
      <div :class="['sheji-page', mood ? '' : 'bigger']">
        <div class="result-box">
          <vue-drag-select v-model="selectedArr" value-key="photoName" :item-margin="[8, 8, 60, 8]" ref="dragSelectArr">
            <template v-for="(item, index) in this.ky_imgList">
              <drag-select-option :key="index" :value="item" :item-index="index" ref="dragSelectOption">
                <div
                  class="item-self"
                  @contextmenu.prevent="rightPopup($event,item,index)"
                ...
                <div class="text-secondary ellipsis-2 fs-14">{{ item.id }}</div>
    ...
    <Modal class="" :mask="false" ref="index-right-modal" v-model="imgPopup" :closable="false" width="350">
      ...
      <div>
        <div v-for="(k, i) in this.ky_lookArr" :key="i" class="checkClass">
          ...
          <!-- <span v-show="k.biaoji" @click="linkFun" style="cursor:pointer;color:dodgerblue" >标记说明</span> -->
        </div>
      </div>
      <div slot="footer"></div>
    </Modal>
    ...
    >
      ...
      <div class="px-1 py-2">
        <div v-if="curImgId">当前相片：{{this.currentImgObj.id}}</div>
    ...
    <!--图片预览 定义ref 用来传值-->
    <!-- <imgViewer ref="allviewer"></imgViewer> -->
  </div>
</template>
  ...
  methods: {
    ...
    rightPopup (e, item, i) {
      ...
      this.$nextTick(() => {
        ...
        // 这里解决checkbox选择其中一张图片后，其他未选的图片也会为true；
        // k2.num这个数组包含选中数组的url，就为true;否则为false;
    ...
    checkboxChangeFun (e, k, i) {
      if (e) { // 当设计需要修改为true
        this.selectedArr.forEach((v) => {
          this.ky_imgList.forEach((v2) => {
            if (v.id === v2.id) {
              this.ky_lookArr.forEach((v3) => {
                if (v3.title === k.title) {
                  v2.photoNames.push(k.title)
                  Array.from(new Set(v2.photoNames))
                  v2.isModify = true
                  v2.modify = 2
                  v3.isSelected = true
                  v3.num.push(v2.url)
                ...
                if (v3.title !== k.title) {
                  v2.isModify = false
                  v3.isSelected = false
                  v3.num.splice(v3.num.indexOf(v2.url), 1)
                }
              })
            }
          })
    ...
    /**
     * 根据id匹配
     * v-modal:this.singleNote;
     * 点击确定按钮后关闭，v-modal赋值给当前图片留言板；
     */
    opinionOk () {
      this.selectedArr.forEach((k) => {
        if (k.id == this.currentImgObj.id) {
    ...
    showImgFun (k, i) {
      console.log('all双击图片')
      // this.$refs.allviewer.show(k);
      this.$nextTick(() => {
        bus.$emit('allviewer', k)
      })
    },
    /**
     * 主要是取消右键弹窗用的
  ...
  watch: {
    ...
    ky_imgList: {
      handler () {
        ...
        sessionStorage.setItem('ky_okArr', JSON.stringify(okArr))
        sessionStorage.setItem('ky_modifyArr', JSON.stringify(modifyArr))
      },
      deep: true
    }
  },
