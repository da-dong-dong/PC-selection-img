<template>
  <div class="all" @click="allFun($event)">
    <div>
      <div :class="['sheji-page', mood ? '' : 'bigger']">
        <div class="result-box">
          <vue-drag-select v-model="selectedArr" value-key="photoName" :item-margin="[8, 8, 60, 8]" ref="dragSelectArr">
            <template v-for="(item, index) in this.ky_okArr">
              <drag-select-option :key="index" :value="item" :item-index="index" ref="dragSelectOption">
                <div
                  class="item-self"
                  @contextmenu.prevent="rightPopup($event)"
    ...
    <!--图片预览 定义ref 用来传值-->
    <!-- <imgViewer ref="okviewer"></imgViewer> -->
  </div>
</template>
      ...
      if (this.selectedArr.length == 0) {
        ...
        this.$Modal.confirm({
          ...
          onCancel: () => {
            ...
            // console.log(this.$refs.dragSelectOption);
          }
        })
      }
    },
